...
멘트. 새로운 OpenAI 에이전트 SDK 패턴: 하네스는 컴퓨팅과 별개입니다. 신뢰할 수 있는 애플리케이션 런타임은 에이전트 루프, 도구, 승인, 비밀, 가드레일, 추적 및 비즈니스 시스템 액세스를 소유합니다. 가이드가 끝나면 실행, 안전, 상태 및 소유권을 명시적으로 지정하면서 원래 Claude 앱 동작을 유지하는 마이그레이션된 앱이 있어야 합니다. 도구로서의 아키텍처 샌드박스 Claude Agent SDK 애플리케이션은 Claude Code 운영 모델을 따릅니다. 샌드박스는 에이전트의 기본 작업 공간입니다. 에이전트 루프, 도구, 파일 시스템 및 실행 환경은 종종 함께 존재합니다. 게이트웨이는 샌드박스와 내부 시스템 사이에 위치할 수 있으므로 샌드박스는 민감한 자격 증명을 직접 보유하지 않고도 제어된 요청을 수행할 수 있습니다. <그림 <img src="assets/claude 에이전트 SDK 패턴.p
