...
- 원문 문서: https://github.com/openai/openai-cookbook/blob/main/examples/agents_sdk/migrate-from-claude-agent-sdk/README.md
- 미러 경로: `examples/agents_sdk/migrate-from-claude-agent-sdk/README.md`

## 한글 요약

Claude Agent SDK에서 OpenAI Agents SDK로 마이그레이션 마이그레이션 이유 업데이트된 OpenAI Agents SDK는 도구, 파일, 메모리, 승인 및 샌드박스 컴퓨팅 전반에 걸쳐 조정할 수 있는 에이전트 구축을 위한 모델 네이티브 하네스를 팀에 제공합니다. 오픈 소스이므로 팀에서 런타임 동작을 검사하고 맞춤 설정할 수 있습니다. 각 모델 통합을 중심으로 이러한 부분을 다시 구축하는 대신 핵심 에이전트 루프를 표시하고 사용자 정의할 수 있도록 유지하는 표준 SDK를 사용할 수 있습니다. 많은 Claude Agent SDK 애플리케이션은 Claude Code 운영 모델을 따릅니다. 즉, 프로젝트 지침 로드, 기술 로드, 파일/셸/웹 도구 노출, 선택적으로 하위 에이전트 정의, Claude 하네스가 작업 공간 내에서 작동하도록 합니다. 새로운 OpenAI Agents SDK는 팀에게 에이전트 하네스와 컴퓨팅 환경 간의 보다 명확한 분할을 제공합니다. 가장 중요한 아키텍처 차이점: Claude Agent SDK 패턴: 하네스는 샌드박스 내부에서 실행됩니다. 샌드박스에는 에이전트 루프, MCP/도구, 파일 시스템 액세스 및 실행 환경이 포함되어 있습니다.

## 핵심 발췌

설치. 새로운 OpenAI 에이전트 SDK 패턴: 하네스는 컴퓨팅과 별개입니다. 신뢰할 수 있는 애플리케이션 런타임은 에이전트 루프, 도구, 승인, 비밀, 가드레일, 추적 및 비즈니스 시스템 액세스를 소유합니다. 가이드가 끝나면 실행, 안전, 상태 및 소유권을 명시적으로 지정하면서 원래 Claude 앱 동작을 유지하는 마이그레이션된 앱이 있어야 합니다. 도구로서의 아키텍처 샌드박스 Claude Agent SDK 애플리케이션은 Claude Code 운영 모델을 따릅니다. 샌드박스는 에이전트의 기본 작업 공간입니다. 에이전트 루프, 도구, 파일 시스템 및 실행 환경은 종종 함께 존재합니다. 게이트웨이는 샌드박스와 내부 시스템 사이에 위치할 수 있으므로 샌드박스는 민감한 자격 증명을 직접 보유하지 않고도 제어된 요청을 수행할 수 있습니다. <그림 <img src="assets/claude 에이전트 SDK 패턴.p
