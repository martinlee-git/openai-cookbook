# How_to_build_an_agent_with_the_node_sdk

- 원문 저장소: `openai/openai-cookbook`
- 미러 저장소: `martinlee-git/openai-cookbook`
- 원문 문서: https://github.com/openai/openai-cookbook/blob/main/examples/How_to_build_an_agent_with_the_node_sdk.mdx
- 미러 경로: `examples/How_to_build_an_agent_with_the_node_sdk.mdx`

## 한글 요약

Node.js SDK를 사용하여 에이전트를 구축하는 방법 OpenAI 기능을 사용하면 앱이 사용자 입력에 따라 조치를 취할 수 있습니다. 이는 예를 들어 사용자를 대신하여 웹 검색, 이메일 전송, 티켓 예약 등을 수행할 수 있어 일반 챗봇보다 더 강력하다는 것을 의미합니다. 이 튜토리얼에서는 최신 버전의 Node.js SDK와 함께 OpenAI 기능을 사용하는 앱을 빌드합니다. 앱은 브라우저에서 실행되므로 로컬에서 따라가려면 코드 편집기와 VS Code Live Server만 있으면 됩니다. 또는 Scrimba의 이 코드 플레이그라운드를 통해 브라우저에서 직접 코드를 작성하세요. 우리 앱은 귀하가 거주하는 지역에서 활동을 찾는 데 도움을 주는 간단한 에이전트입니다. getLocation() 및 getCurrentWeather()라는 두 가지 함수에 액세스할 수 있습니다. 즉, 사용자의 위치와 현재 날씨를 파악할 수 있습니다. 이 시점에서는 OpenAI가 사용자를 위해 어떤 코드도 실행하지 않는다는 점을 이해하는 것이 중요합니다. 주어진 시나리오에서 어떤 기능을 사용해야 하는지 앱에 알려줄 뿐입니다.

## 핵심 발췌

를 호출한 다음 이를 앱에 맡겨 호출합니다. 에이전트가 사용자의 위치와 날씨를 파악하면 GPT의 내부 지식을 사용하여 적합한 지역 활동을 제안합니다. SDK 가져오기 및 OpenAI로 인증 먼저 JavaScript 파일 상단에서 OpenAI SDK를 가져오고 환경 변수로 저장한 API 키로 인증합니다. 우리는 Scrimba의 브라우저 환경에서 코드를 실행하고 있으므로 클라이언트 측 API 요청과 관련된 위험을 이해하고 있는지 확인하기 위해angerlyAllowBrowser:true도 설정해야 합니다. 이러한 요청을 프로덕션 앱의 노드 서버로 이동해야 한다는 점에 유의하세요. 두 가지 함수 만들기 다음으로 두 가지 함수를 만듭니다. 첫 번째 getLocation은 IP API를 사용하여 위치를 가져옵니다.
