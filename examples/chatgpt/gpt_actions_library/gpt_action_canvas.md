...
의견, 학생 협업. Canvas REST API를 통해 Canvas는 ChatGPT와 같은 AI 기반 도구를 포함하여 타사 애플리케이션과의 광범위한 사용자 정의 및 통합을 허용합니다. ChatGPT의 Canvas를 사용한 사용자 지정 작업을 통해 교육자는 AI를 활용하여 코스 콘텐츠를 향상하고 작업을 자동화하며 학생에게 맞춤형 학습 여정을 제공할 수 있습니다. 예를 들어 활성 코스를 기반으로 하는 가상 조교가 포함됩니다. 이 기능은 교육적 대화를 만들기 위해 Canvas에서 정보를 가져오는 데 매우 적합합니다. 사용자 정의 작업이 포함된 ChatGPT는 전체 Canvas 경험을 자동화하거나 Canvas 앱에서 완료하는 데 더 적합한 여러 기능을 대체하기 위한 것이 아닙니다. ChatGPT에서 Canvas로 인증 Authent에 대한 일반적인 개요
