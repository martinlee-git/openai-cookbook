...
문서를 좋게 만드는 요소 문서는 다른 사람의 머리 속에 유용한 정보를 넣어줍니다. 더 나은 문서를 작성하려면 다음 팁을 따르세요. 문서를 쉽게 훑어볼 수 있도록 위에서 아래로 선형적으로 읽는 독자는 거의 없습니다. 그들은 이리저리 뛰어다니며 어떤 부분이 문제를 해결하는지 평가하려고 합니다. 검색 시간을 줄이고 성공 확률을 높이려면 문서를 쉽게 훑어볼 수 있도록 만드세요. 콘텐츠를 제목이 있는 섹션으로 분할합니다. 섹션 제목은 독자에게 집중할지 아니면 다음 단계로 나아갈지 알려주는 표지판 역할을 합니다. 추상 명사보다는 정보를 제공하는 문장이 포함된 제목을 선호하세요. 예를 들어, '결과'와 같은 제목을 사용하는 경우 독자는 실제로 결과가 무엇인지 알아보려면 다음 텍스트를 읽어야 합니다. 반면에 "스트리밍으로 첫 번째 토큰까지의 시간이 50% 단축되었습니다"라는 제목을 사용하면 추가 홉의 부담 없이 독자에게 즉시 정보를 제공합니다. 목차를 포함하세요. 목차는 해시 맵이 더 빠르게 조회되는 것과 유사하게 독자가 정보를 더 빠르게 찾는 데 도움이 됩니다.
