...
ChatGPT에서 작업공간 에이전트를 구축하여 반복 가능한 종단 간 작업을 완료합니다. 이 요리책은 ChatGPT 작업공간 에이전트를 구축하는 방법을 보여줍니다. 현재 ChatGPT Business, Enterprise 및 Edu 고객을 위한 연구 미리 보기로 제공되는 작업 공간 에이전트는 도구 전반에 걸쳐 작업하여 고부가가치 작업을 완료할 수 있는 GPT의 발전된 형태로 팀이 더 빠르게 움직일 수 있도록 도와줍니다. 이 요리책에서는 대화 빌더를 통해 작업 공간 에이전트를 생성하고, 팀의 달력 및 지식 기반에 대한 액세스 권한을 부여하고, 에이전트의 출력을 작성하는 데 도움이 되는 간단한 쓰기 기술을 추가합니다. 그런 다음 팀 에이전트를 테스트하고 반복 일정에 따라 실행되도록 예약한 다음 팀원과 공유합니다. 소개 ChatGPT 작업 영역 에이전트는 ChatGPT 전체에서 반복 가능한 워크플로를 실행하고 독립적으로 종단 간 작업을 완료할 수 있는 공유 에이전트입니다. 연결된 앱을 사용하고, 기술을 따르고, 일정에 따라 실행하고, 작업 공간에서 동료와 공유할 수 있습니다. 이 요리책에서는 다음과 같은 작업공간 에이전트를 구축하겠습니다.

## 핵심 발췌

영업팀을 지원합니다. 영업팀은 다가오는 고객 회의를 준비하기 위해 계정 메모, 최근 고객 피드백, 제품 사용 지표 및 회사 뉴스를 수동으로 가져오는 데 시간을 보내는 경우가 많습니다. 최신 고객 데이터를 바탕으로 예정된 고객 통화에 앞서 회의 요약을 자동으로 생성하여 영업 팀이 영업 회의를 보다 효율적으로 준비할 수 있도록 지원하는 작업 공간 에이전트를 구축할 것입니다. 구체적으로 말하면 우리 에이전트는 매일 내일의 고객 회의를 확인하고, SharePoint에서 최근 계정 컨텍스트를 수집하고, 각 참석자에 대한 최근 회사 뉴스와 배경 정보를 웹에서 검색합니다. 상담원이 회의 개요를 생성하고 이를 SharePoint 문서로 저장한 후 요약 내용을 보내드립니다. 에이전트가 배포되면 매일 자동으로 실행됩니다.
