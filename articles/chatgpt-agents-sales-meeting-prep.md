...
ChatGPT에서 작업공간 에이전트를 구축하여 반복 가능한 종단 간 작업을 완료합니다. 이 요리책은 ChatGPT 작업공간 에이전트를 구축하는 방법을 보여줍니다. 현재 ChatGPT Business, Enterprise 및 Edu 고객을 위한 연구 미리 보기로 제공되는 작업 공간 에이전트는 도구 전반에 걸쳐 작업하여 고부가가치 작업을 완료할 수 있는 GPT의 발전된 형태로 팀이 더 빠르게 움직일 수 있도록 도와줍니다. 이 요리책에서는 대화 빌더를 통해 작업 공간 에이전트를 생성하고, 팀의 달력 및 지식 기반에 대한 액세스 권한을 부여하고, 에이전트의 출력을 작성하는 데 도움이 되는 간단한 쓰기 기술을 추가합니다. 그런 다음 팀 에이전트를 테스트하고 반복 일정에 따라 실행되도록 예약한 후 팀원과 공유합니다. 소개 ChatGPT 작업 공간 에이전트는 ChatGPT 전체에서 반복 가능한 워크플로를 실행하고 독립적으로 종단 간 작업을 완료할 수 있는 공유 에이전트입니다. 연결된 앱을 사용하고, 기술을 따르고, 일정에 따라 실행하고, 작업 공간에서 동료와 공유할 수 있습니다. 이 요리책에서는 다음과 같은 작업공간 에이전트를 구축하겠습니다.
